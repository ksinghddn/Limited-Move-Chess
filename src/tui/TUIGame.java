...
public class TUIGame extends Game {
	...
	public String play(boolean verbose) throws IOException {
		handleCommand("new");
		ThreadLocal<Integer> no_of_moves = new ThreadLocal<Integer>();// added by kuldeep	
		no_of_moves.set(0);// added by kuldeep
		int m=0;
		while (true) {
			// Print last move
			
			no_of_moves.set(no_of_moves.get()+1); // Added by kuldeep
			/*if(no_of_moves.get()%50==0)
			{
			//System.out.println("No. of moves"+no_of_moves);
			}*/
