...
public class ChessController {
	...
	class SearchListener implements Search.Listener {
		...
		private void setSearchInfo() {
			...
			buf.append(String.format(Locale.US,
					"d:%d %d:%s t:%.2f n:%d nps:%d", currDepth, currMoveNr,
					currMove, currTime / 1000.0, currNodes, currNps));
		...
		public void notifyPV(int depth, int score, int time, long nodes,
			...
			for (Move m : pv) {
				buf.append(String.format(Locale.US, " %s",
	...
	public final void newGame(boolean humanIsWhite, int ttLogSize,
		...
		// CHANGE HERE TO AFFECT GUI PLAY
		// computerPlayer = new ComputerPlayer(verbose,
		// com.rahul.phoenix.tui.TestMatch.testGenes);
		computerPlayer = new ComputerPlayer(verbose);
	...
	public String getPGN() {
		...
		pgn.append(String.format(Locale.US, "[Date \"%04d.%02d.%02d\"]%n",
				year, month, day));
		String white = "Player";
		...
		pgn.append(String.format(Locale.US, "[Result \"%s\"]%n",
		...
		if (game.pos.whiteMove != humanIsWhite) {
			if (computerThread == null) {
				Runnable run = new Runnable() {
					public void run() {
						computerPlayer.timeLimit(gui.timeLimit(),
								gui.timeLimit(), gui.randomMode());
						final String cmd = computerPlayer.getCommand(
								new Position(game.pos), game.haveDrawOffer(),
								game.getHistory());
						gui.runOnUIThread(new Runnable() {
							public void run() {
				...
				if (threadStack > 0) {
					...
					computerThread = new Thread(tg, run, "searcher",
							threadStack);
				} else {
