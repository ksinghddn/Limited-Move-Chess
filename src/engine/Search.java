...
public class Search {
	...
	KillerTable kt;
	History ht;
	long[] posHashList; // List of hashes for previous positions up to the last
						// "zeroing" move.
	int posHashListSize; // Number of used entries in posHashList
	int posHashFirstNew; // First entry in posHashList that has not been played
							// OTB.
	...
	boolean searchNeedMoreTime; // True if negaScout should use up to
								// maxTimeMillis time.
	private long maxNodes; // Maximum number of nodes to search (approximately)
	int nodesToGo; // Number of nodes until next time check
	public int nodesBetweenTimeCheck = 5000; // How often to check remaining
												// time
	...
	public final static int UNKNOWN_SCORE = -32767; // Represents unknown static
													// eval score
	...
	public interface Listener {
		...
		public void notifyPV(int depth, int score, int time, long nodes,
				int nps, boolean isMate, boolean upperBound,
	...
	final public Move iterativeDeepening(MoveGen.MoveList scMovesIn,
		...
		try {
			for (int depthS = plyScale;; depthS += plyScale, firstIteration = false) {
				...
				int aspirationDelta = (Math.abs(bestScoreLastIter) <= MATE0 / 2) ? 20
						: 1000;
				int alpha = firstIteration ? -Search.MATE0 : Math.max(
						bestScoreLastIter - aspirationDelta, -Search.MATE0);
				...
				for (int mi = 0; mi < scMoves.length; mi++) {
					...
					if (firstIteration) {
						...
						beta = (mi == 0) ? Math.min(bestScoreLastIter
								+ aspirationDelta, Search.MATE0) : alpha + 1;
					}
					...
					sti.lmr = lmrS;
					sti.nodeIdx = -1;
					int score = -negaScout(-beta, -alpha, 1, depthS - lmrS
							- plyScale, -1, givesCheck);
					if ((lmrS > 0) && (score > alpha)) {
						sti.lmr = 0;
						score = -negaScout(-beta, -alpha, 1, depthS - plyScale,
								-1, givesCheck);
					}
					...
					pos.unMakeMove(m, ui);
					{
						...
						m.score = score;
						tt.insert(pos.historyHash(), m, type, 0, depthS,
								UNKNOWN_SCORE);
					}
					if (score >= beta) {
						...
						while (score >= beta) {
							...
							posHashList[posHashListSize++] = pos.zobristHash();
							pos.makeMove(m, ui);
							int score2 = -negaScout(-beta, -score, 1, depthS
									- plyScale, -1, givesCheck);
						...
						while (score <= alpha) {
							...
							posHashList[posHashListSize++] = pos.zobristHash();
							pos.makeMove(m, ui);
							score = -negaScout(-score, -alpha, 1, depthS
									- plyScale, -1, givesCheck);
							nodesThisMove += nodes + qNodes;
							posHashListSize--;
							pos.unMakeMove(m, ui);
					...
					if (!firstIteration) {
						long timeLimit = needMoreTime ? maxTimeMillis
								: minTimeMillis;
						if (timeLimit >= 0) {
				...
				if (firstIteration) {
					...
					bestMove = scMoves[0].move;
					notifyPV(depthS / plyScale, bestMove.score, false, false,
							bestMove);
				}
				long tNow = System.currentTimeMillis();
				if (verbose) {
				...
				if (!firstIteration) {
					// Moves that were hard to search should be searched early
					// in the next iteration
					Arrays.sort(scMoves, 1, scMoves.length,
	...
	private final void notifyPV(int depth, int score, boolean uBound,
			boolean lBound, Move m) {
		if (listener != null) {
			...
			listener.notifyPV(depth, score, time, totalNodes, nps, isMate,
					uBound, lBound, pv);
		}
	}
	 ...
	 */
	final public int negaScout(int alpha, int beta, int ply, int depth,
			int recaptureSquare, final boolean inCheck) throws StopSearch {
		if (log != null) {
			SearchTreeInfo sti = searchTreeInfo[ply - 1];
			long idx = log.logNodeStart(sti.nodeIdx, sti.currentMove, alpha,
					beta, ply, depth / plyScale);
		...
		if (verbose) {
			if (ply < 20)
				nodesPlyVec[ply]++;
			if (depth < 20 * plyScale)
				nodesDepthVec[depth / plyScale]++;
		}
		...
		if (canClaimDraw50(pos)) {
			if (MoveGen.canTakeKing(pos)) {
				int score = MATE0 - ply;
				if (log != null)
					log.logNodeEnd(searchTreeInfo[ply].nodeIdx, score,
							TTEntry.T_EXACT, UNKNOWN_SCORE, hKey);
				return score;
			}
			if (inCheck) {
				MoveGen.MoveList moves = moveGen.pseudoLegalMoves(pos);
				MoveGen.removeIllegal(pos, moves);
				if (moves.size == 0) { // Can't claim draw if already check
										// mated.
			...
			if (log != null)
				log.logNodeEnd(searchTreeInfo[ply].nodeIdx, 0, TTEntry.T_EXACT,
						UNKNOWN_SCORE, hKey);
			return 0;
		}
		...
		if (ent.type != TTEntry.T_EMPTY) {
			...
			if ((beta == alpha + 1)
					&& ((eDepth >= depth) || (eDepth >= plyToMate * plyScale))) {
				if ((ent.type == TTEntry.T_EXACT) || (ent.type == TTEntry.T_GE)
						&& (score >= beta) || (ent.type == TTEntry.T_LE)
						&& (score <= alpha)) {
					if (score >= beta) {
						hashMove = sti.hashMove;
						if ((hashMove != null)
								&& (hashMove.from != hashMove.to))
		...
		if (!inCheck && (depth < 5 * plyScale) && (posExtend == 0)
			...
			if (mtrlOk) {
				...
				if (evalScore - margin >= beta) {
					...
					tt.insert(hKey, emptyMove, TTEntry.T_GE, ply, depth,
							evalScore);
					if (log != null)
						log.logNodeEnd(sti.nodeIdx, evalScore - margin,
								TTEntry.T_GE, evalScore, hKey);
		...
		// FIXME! Try null-move verification in late endgames. See loss in round
		// 21.
		...
		if ((depth >= 3 * plyScale) && !inCheck && sti.allowNullMove
			...
			if (MoveGen.canTakeKing(pos)) {
				int score = MATE0 - ply;
				if (log != null)
					log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_EXACT,
							evalScore, hKey);
				return score;
			}
			boolean nullOk;
			...
			if (nullOk) {
				final int R = (depth > 6 * plyScale) ? 4 * plyScale
						: 3 * plyScale;
				...
				int score = -negaScout(-beta, -(beta - 1), ply + 1, depth - R,
						-1, false);
				...
				if (score >= beta) {
					...
					tt.insert(hKey, emptyMove, TTEntry.T_GE, ply, depth,
							evalScore);
					if (log != null)
						log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_GE,
								evalScore, hKey);
					return score;
				} else {
					if ((searchTreeInfo[ply - 1].lmr > 0)
						...
						Move m2 = searchTreeInfo[ply + 1].bestMove; // threat
																	// move
						if (relatedMoves(m1, m2)) {
							// if the threat move was made possible by a reduced
							// move on the previous ply, the reduction was
							// unsafe.
							// Return alpha to trigger a non-reduced re-search.
							if (log != null)
								log.logNodeEnd(sti.nodeIdx, alpha,
										TTEntry.T_LE, evalScore, hKey);
							return alpha;
						}
					}
				}
		...
		if ((depth > 4 * plyScale)
				&& ((hashMove == null) || (hashMove.from == hashMove.to))) {
		...
		for (int mi = 0; mi < moves.size; mi++) {
			...
			Move m = moves.m[mi];
			if (pos.getPiece(m.to) == (pos.whiteMove ? Piece.BKING
					: Piece.WKING)) {
				moveGen.returnMoveList(moves);
				int score = MATE0 - ply;
				if (log != null)
			...
			boolean mayReduce = (m.score < 53) && (!isCapture || m.score < 0)
					&& !isPromotion;
			boolean givesCheck = MoveGen.givesCheck(pos, m);
			boolean doFutility = false;
			if (mayReduce && haveLegalMoves && !givesCheck
					&& !passedPawnPush(pos, m)) {
				if ((Math.abs(alpha) <= MATE0 / 2)
						&& (Math.abs(beta) <= MATE0 / 2)) {
					int moveCountLimit;
					if (depth <= plyScale)
						moveCountLimit = 3;
			...
			if (doFutility) {
				...
				if (posExtend == 0) {
					...
					if ((m.to == recaptureSquare)) {
						if (sVal == Integer.MIN_VALUE)
							sVal = SEE(m);
						int tVal = Evaluate.pieceValue[pos.getPiece(m.to)];
						if (sVal > tVal - pV / 2)
							moveExtend = plyScale;
					}
					if ((moveExtend < plyScale) && isCapture
							&& (pos.wMtrlPawns + pos.bMtrlPawns > pV)) {
						// Extend if going into pawn endgame
						int capVal = Evaluate.pieceValue[pos.getPiece(m.to)];
						if (pos.whiteMove) {
							if ((pos.wMtrl == pos.wMtrlPawns)
									&& (pos.bMtrl - pos.bMtrlPawns == capVal))
								moveExtend = plyScale;
						} else {
							if ((pos.bMtrl == pos.bMtrlPawns)
									&& (pos.wMtrl - pos.wMtrlPawns == capVal))
								moveExtend = plyScale;
						}
					}
				}
				...
				if (isCapture && (givesCheck || (depth + extend) > plyScale)) {
					// Compute recapture target square, but only if we are not
					// going
				...
				sti.lmr = lmr;
				score = -negaScout(-b, -alpha, ply + 1, newDepth,
						newCaptureSquare, givesCheck);
				if (((lmr > 0) && (score > alpha))
						|| ((score > alpha) && (score < beta) && (b != beta) && (score != illegalScore))) {
					sti.lmr = 0;
					newDepth += lmr;
					score = -negaScout(-beta, -alpha, ply + 1, newDepth,
							newCaptureSquare, givesCheck);
				}
			...
			if (alpha >= beta) {
				...
				moveGen.returnMoveList(moves);
				if (log != null)
					log.logNodeEnd(sti.nodeIdx, alpha, TTEntry.T_GE, evalScore,
							hKey);
				return alpha;
			}
			b = alpha + 1;
		...
		if (!haveLegalMoves && !inCheck) {
			moveGen.returnMoveList(moves);
			if (log != null)
				log.logNodeEnd(sti.nodeIdx, 0, TTEntry.T_EXACT, evalScore, hKey);
			return 0; // Stale-mate
		}
		if (bestMove >= 0) {
			tt.insert(hKey, moves.m[bestMove], TTEntry.T_EXACT, ply, depth,
					evalScore);
			if (log != null)
				log.logNodeEnd(sti.nodeIdx, bestScore, TTEntry.T_EXACT,
						evalScore, hKey);
		} else {
			emptyMove.score = bestScore;
			tt.insert(hKey, emptyMove, TTEntry.T_LE, ply, depth, evalScore);
			if (log != null)
				log.logNodeEnd(sti.nodeIdx, bestScore, TTEntry.T_LE, evalScore,
						hKey);
		}
		moveGen.returnMoveList(moves);
		return bestScore;
	...
	/**
	 * Return true if move should be skipped in order to make engine play
	 * weaker.
	 */
	private final boolean weakPlaySkipMove(Position pos, Move m, int ply) {
		...
		double effPly = ply
				* Evaluate.interpolate(pos.wMtrl + pos.bMtrl, 0, 30,
						Evaluate.qV * 4, 100) * 1e-2;
		double t = effPly + offs;
		double p = 1 / (1 + Math.exp(t)); // Probability to "see" move
		boolean easyMove = ((pos.getPiece(m.to) != Piece.EMPTY) || (ply < 2) || (searchTreeInfo[ply - 2].currentMove.to == m.from));
		if (easyMove)
			p = 1 - (1 - p) * (1 - p);
		if (rnd > p)
			return true;
	 ...
	 */
	final private int quiesce(int alpha, int beta, int ply, int depth,
		...
		for (int mi = 0; mi < moves.size; mi++) {
			if (mi < 8) {
				// If the first 8 moves didn't fail high, this is probably an
				// ALL-node,
				// so spending more effort on move ordering is probably wasted
				// time.
				selectBest(moves, mi);
			}
			Move m = moves.m[mi];
			...
			if (inCheck) {
				...
				if ((pos.getPiece(m.to) == Piece.EMPTY)
					...
					if (!givesCheck)
						continue;
					if (negSEE(m)) // Needed because m.score is not computed for
									// non-captures
						continue;
				} else {
					if (negSEE(m))
					...
					if (optimisticScore < alpha) { // Delta pruning
						if ((pos.wMtrlPawns > 0)
								&& (pos.wMtrl > capt + pos.wMtrlPawns)
								&& (pos.bMtrlPawns > 0)
								&& (pos.bMtrl > capt + pos.bMtrlPawns)) {
							if (depth - 1 > -2) {
								givesCheck = MoveGen.givesCheck(pos, m);
								givesCheckComputed = true;
							}
	 ...
	 * 
	 * @return SEE score for m. Positive value is good for the side that makes
	 *         the first move.
	 */
	final public int SEE(Move m) {
		...
		while (true) {
			...
			long atk;
			if (white) {
				atk = BitBoard.bPawnAttacks[square]
						& pos.pieceTypeBB[Piece.WPAWN] & occupied;
				if (atk != 0) {
					bestValue = Evaluate.pV;
				} else {
					atk = BitBoard.knightAttacks[square]
							& pos.pieceTypeBB[Piece.WKNIGHT] & occupied;
					if (atk != 0) {
						bestValue = Evaluate.nV;
					} else {
						long bAtk = BitBoard.bishopAttacks(square, occupied)
								& occupied;
						atk = bAtk & pos.pieceTypeBB[Piece.WBISHOP];
						if (atk != 0) {
							bestValue = Evaluate.bV;
						} else {
							long rAtk = BitBoard.rookAttacks(square, occupied)
									& occupied;
							...
							if (atk != 0) {
								...
								if (atk != 0) {
									...
									atk = BitBoard.kingAttacks[square]
											& pos.pieceTypeBB[Piece.WKING]
											& occupied;
									if (atk != 0) {
										bestValue = kV;
									} else {
				...
				atk = BitBoard.wPawnAttacks[square]
						& pos.pieceTypeBB[Piece.BPAWN] & occupied;
				if (atk != 0) {
					bestValue = Evaluate.pV;
				} else {
					atk = BitBoard.knightAttacks[square]
							& pos.pieceTypeBB[Piece.BKNIGHT] & occupied;
					if (atk != 0) {
						bestValue = Evaluate.nV;
					} else {
						long bAtk = BitBoard.bishopAttacks(square, occupied)
	...
	/**
	 * Compute scores for each move in a move list, using SEE, killer and
	 * history information.
	 * 
	 * @param moves
	 *            List of moves to score.
	 */
	...
	final void scoreMoveList(MoveGen.MoveList moves, int ply, int startIdx) {
		for (int i = startIdx; i < moves.size; i++) {
			...
			if (isCapture) {
				...
				score = Evaluate.pieceValue[v] / 10 * 1000
						- Evaluate.pieceValue[a] / 10;
				if (seeScore > 0)
					score += 2000000;
				else if (seeScore == 0)
					score += 1000000;
	...
	/**
	 * If hashMove exists in the move list, move the hash move to the front of
	 * the list.
	 */
	...
	public final static boolean canClaimDrawRep(Position pos,
